...
def fakeresés(feladat,perem):
    perem.append(Csúcs(feladat.kezdo))
    while perem:
        csúcs = perem.pop()
...
def gráfkeresés(feladat, perem):
    kifejtési_sor=set() #ugyanaz, mint a lista, viszont mindegyik elem különböző, nem lehet duplikált
    perem.append(Csúcs(feladat.kezdo))
    while perem: # amíg a perem nem üres
        csúcs = perem.pop()
        if feladat.célteszt(csúcs.állapot):
            return csúcs
...
def best_first(feladat, heurisztika):
    return gráfkeresés(feladat, RendezettLista(heurisztika))
